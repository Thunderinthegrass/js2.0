//код урока
let a = 'Здравствуйте, люди!';
let aa = a.split(' ');//возвратит массив из двух слов
console.log(aa);
let ab = a.split('', 8);
console.log(ab);//возвратит массив из символов, из которых состоит фраза, включая пробелы и знаки препинания. Второй аргумент указывает, сколько элементов будет в возвращаемом массиве, все остальные обрежутся. Если не указать первый аргумент - separator, то возвратится массив с одним элементом - исходной строкой. Если не указать второй - limit, то просто ничего не обрежется. 

//Метод join() объединяет все элементы массива (или массивоподобного объекта) в строку. arr.join([separator])
...
//forEach не возвращает новый массив, он применяет функцию к каждому элементу, если нужен новый массив - он создается внутри функции и уже туда складывается всё, что нужно. Но он также не изменяет исходный массив.
